# Programación II

## Temario

- [x] [Repaso de programación estructurada](https://github.com/mmasias/prg1)
- [x] [Introducción](documentos/01-introduccion/README.md)
  - [x] [Bases de la programación](documentos/01-introduccion/basesProgramacion.md)
  - [x] [Evolución de los lenguajes de programación](documentos/01-introduccion/evolucion.md)
  - [x] [La programación orientada a objetos](documentos/01-introduccion/poo.md)
- Orientación a objetos
  - Programación basada en objetos
    - [x] [Clases y objetos](documentos/02-vistas/clasesObjetos.md)
      - [x] [Introducción](documentos/02-vistas/introVistas.md)
      - [x] [Vista pública de las clases](documentos/02-vistas/vistaPublicaClases.md)
      - [x] [Vista pública de los objetos](documentos/02-vistas/vistaPublicaObjetos.md)
      - [ ] [Vista privada de las clases](documentos/vistaPrivadaClases.md)
      - [ ] [Vista privada de los objetos](documentos/vistaPrivadaObjetos.md)
      - [ ] Miembros de clase
  - [ ] [Colaboración entre objetos: HOOD (*a.k.a. FotB*)](https://github.com/mmasias/24-25-PRG2/discussions/312)
    - [ ] Desarrollo de aplicaciones usando HOOD:
      - [ ] [Tic Tac Toe](https://github.com/mmasias/TicTacToe)
      - [ ] [Klondike](https://github.com/mmasias/24-25-pyKlondike)
      - [ ] [Blackjack (*a.k.a. 21*)](https://github.com/mmasias/24-25-pyKlondike/tree/XXI)
  - Programación orientada a objetos
    - [ ] [Relación de Herencia](documentos/herencia.md)
      - [ ] [Herencia por extensión](documentos/herencia.md#herencia-por-extensi%C3%B3n)
      - [ ] [Clases Abstractas](documentos/herencia.md#clases-abstractas)
      - [ ] [Herencia por Implementación](documentos/herencia.md#herencia-por-implementación-interfaces)
      - [ ] [Limitar la Herencia](documentos/herencia.md#cómo-limitar-la-herencia)
      - [ ] [Beneficios de la Herencia](documentos/herencia.md#beneficios-de-la-herencia)
    - [ ] Polimorfismo
    - [ ] Conversión de Tipos
    - [ ] Herencia y Enumerados

## Temario++

- [ ] Artículos varios recopilados en la [carpeta de documentos](documentos/01-introduccion/README.md).
